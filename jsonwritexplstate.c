...
int callDomoticz(char *data)
{
  ...
  if(curl) {
    ...
    char url[256];
    char *writeData;
    strcpy(url, "http://127.0.0.1:2709/json.htm?type=command&param=customevent&event=VisonicJsonReady&data=");
    strcat(url, data);
...
/* TODO: check if process is already running */
  ...
  //  printf("zonelist: %s\n",zonelist);
    ...
    fprintf(f, "{ \"status\":\"%s\",\"pmaxstatus\":\"%s\",\"readytoarm\":\"%s\",\"sensor\":[",pmaxSystem.status,pmaxSystem.pmaxstatus,pmaxSystem.readytoarm);
